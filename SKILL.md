...
1. `config/config.yaml` (if not found, read `config/config.example.yaml` and tell the user to create `config.yaml`)
2. `config/prefix-rules.md` (if not found, read `config/prefix-rules.example.md` and tell the user to create `prefix-rules.md`)
